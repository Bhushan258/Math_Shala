...
QueStore=[];
QueGen=(questions)=>{
    var No = getRandomInt(11,99);
    while(No%10==0||QueStore.includes(No)){
        No = getRandomInt(11,99);
    }
    QueStore.push(No);
    var first_half =Math.trunc(No/10),second_half=Math.trunc(No%10);
    var choices=[],chotemp=[];
    var qs="{"+(first_half.toString())+","+second_half.toString()+"}";
    var Ans=second_half*10+first_half;
    choices[0]=Ans;
    chotemp[0]=No;
    for (let index = 1; index <=3; index++) {
        var sumte=getRandomInt(11,99);
        while(sumte%10==0|| sumte%11==0 || chotemp.includes(sumte) || choices.includes(sumte)){
            sumte=getRandomInt(11,99);
        }
        chotemp.push(sumte);
        choices.push(sumte);
    }
    shuffle(choices);
    var Que="Create a Number from following digits : "+qs+"<br/> Such as  "+No+"<br/>";
