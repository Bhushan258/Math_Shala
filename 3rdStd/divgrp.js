...
var QueStore=[];
QueGen=(questions)=>{
    var dig = getRandomInt(101,999)
    while (QueStore.includes(dig) || dig%4!=0){
        var dig = getRandomInt(101,999);
    }
    QueStore.push(dig);
    var part =getRandomfromList([1/4,2/4,3/4]);
    if (part==0.25){
        var partt='one-forth (¼)';
    }else if(part==0.5){
        var partt='half (½)';
    }else{
        var partt='three-forth (¾)';
    }
    var choices=[];
    choices[0]=dig*part;
   
    for (let i=1; i<=3; i++){
         var st=getRandomInt(101,999);
         while(choices.includes(st)){
            st=getRandomInt(101,999);
         }
         choices.push(st);
    }
    shuffle(choices);
    var Que="What is  "+partt+" of "+dig+"  ? ";
    var obj={
        question:Que,
        ...
        answer:(choices.indexOf(dig*part)+1),
    };
