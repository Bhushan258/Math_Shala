...
QueGen=(questions)=>{
    ...
    var No=[getRandomfromList(dig)];
    var N=0;
    var temp=getRandomfromList(dig);
    var choices=[];
    while(No.length!=3){
        if (No.includes(temp)){
            var temp=getRandomfromList(dig);
    ...
    sortord=getRandomfromList(["Smallest","Largest"]);
    if (sortord=="Smallest"){
        Ans=[...No.sort((a, b) => a - b)];
    }else{
        Ans=[...No.sort((a, b) => b - a)];
    ...
    for (let index = 0; index < No.length; index++) {
        N+=Ans[index]*(10**(Ans.length-index-1));
    }
    shuffle(No);
    QueStore.push(N);
    Ans=N;
    ...
    shuffle(choices);
    var Que="Create a "+sortord+" Possible number from following digits : "+No+"<br/>";
