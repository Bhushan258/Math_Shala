...
QueStore=[];
QueGen=(questions)=>{
    var first_no = getRandomInt(59,499);
    var step =  getRandomInt(11,299);
    while(QueStore.includes([first_no,step])){
        first_no = getRandomInt(59,499);
        step=getRandomInt(11,299);
    }
    ...
    for (let index = 1; index <=3; index++) {
        var st=getRandomInt(first_no,1000);
        while(choices.includes(st)){
            st=getRandomInt(first_no,1000);
        }
        choices.push(st);
    }
    shuffle(choices);
