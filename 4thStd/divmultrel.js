...
QueGen=(questions)=>{
    var x = getRandomInt(100,300);
    var y = getRandomInt(10,50);
    var z = x * y;
    var choices=[];
    var sign=getRandomfromList([' X ',' &divide '])
    if (sign==' X ') {
        choices[0]=getRandomfromList([x,y]);
        var eq1=x+sign+y+' = '+z;
        var eq2=z+' &divide '+y+' =';
        for (let i=1; i<=3; i++){
            var st=getRandomInt(90,x);
            while(choices.includes(st)){
                st=getRandomInt(90,x);
            }
            choices.push(st);
        }
    } else {
        choices[0]=getRandomfromList([1,2]);
      
            choices[0]=z;
            var eq1=z+sign+y+' = '+x;
            var eq2=x+' X '+y+' =';
            for (let i=1; i<=3; i++){
                var st=getRandomInt(x,z);
                while(choices.includes(st)){
                   st=getRandomInt(x,z);
                }
                choices.push(st);
