...
QueStore=[];
QueGen=(questions)=>{
    var Q=getRandomInt(1001,9999),choices=[],temp=getRandomfromList(['num','word']);
    if (temp=='num'){
        Que="Convert following number into words : </br>"+Q;
        choices[0]=numberToEnglish(Q);
        Ans=numberToEnglish(Q);
        for (let index = 0; index < 4; index++) {
            var st=getRandomInt(1001,9999);
            while(choices.includes(st)){
                st=getRandomInt(1001,9999);            
            }
            choices.push(numberToEnglish(st));
        }
        shuffle(choices)
    }else{
        Que="Convert following words into number : </br>"+numberToEnglish(Q);
        choices[0]=Q;
        Ans=Q;
        for (let index = 0; index < 4; index++) {
            var st=getRandomInt(1001,9999);
