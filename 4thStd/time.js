...
var QueStore=[];
QueGen=(questions)=>{
    var from =getRandomfromList(['hours','minutes','seconds']);
    var to =getRandomfromList(['hours','minutes','seconds']);
    var name=getRandomfromList(['Ram','Sharan','Abhishek']);
    
    if (from=='hours'){
        var dig = getRandomInt(2,14);
        QueStore.push(dig);
        if (to=='hours')
        {
            to=getRandomfromList(['hours','seconds'])
            if (to=='minutes')
            {
            ans=dig*60;
            }
...
if (from=='seconds')
{
    var dig = getRandomInt(100,54000);
    QueStore.push(dig);
	if (to=='hours')
	{
        to='minutes'
    ...
    for (let i=1; i<=3; i++){
         var st=getRandomInt(10,100);
         while(choices.includes(st)){
            st=getRandomInt(10,100);
         }
         choices.push(st);
    }
    shuffle(choices);
    var Que="If "+name+" works for "+dig+" "+from+" then how many "+to+" did he work for ? ";
    var obj={
        question:Que,
